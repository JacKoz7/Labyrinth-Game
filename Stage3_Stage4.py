...
class Game_status:
    def __init__(self, walls, found_labyrinth, winner):
        ...
        self.found_labyrinth = found_labyrinth  # The labyrinth discovered by the opponent
...
class Second_Stage:
    ...
    def __init__(self, treasure, labyrinth, cross):
        ...
        self.labyrinth = labyrinth  # A list containing the labyrinth's positions on the board
        self.cross = cross  # The position of the cross
        self.correct_squares = [cross]  # A list containing the correct squares, initially containing only the cross position
    ...
    def endgame(self, screen, txt, txt2, walls, labyrinth_temp, winner):
        ...
        while True:
            ...
            path_text = self.get_font(45).render('Find the way to the opponent\'s treasure', True, 'Red')
            ...
            if winner is False:
                screen.blit(path_text, path_rect)  # Displaying text if there is no winner
            ...
            if winner:  # If there is a winner
                ...
                winner_button_menu = Button(image=img_button, pos=(700, 500), text_input="Menu", font=self.get_font(65),
                                            base_color='Black', new_color='White')
                play_again_button = Button(image=img_button, pos=(700, 350), text_input="Play Again", font=self.get_font(65),
                                           base_color='Black', new_color='White')
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and cross_drawn and event.button == 1:  # If the left mouse button is depressed.:
                    ...
                    if location is not None and (100 <= mouse_x <= 100 + 10 * 60 and 50 <= mouse_y <= 50 + 10 * 60):
                        ...
                        if any(selected_square in self.get_neighbors(correct_square) for correct_square in
                               self.correct_squares) and not winner:
                            ...
                            if selected_square in self.labyrinth:
                                ...
                                if selected_square not in self.correct_squares:
                                    ...
                                    labyrinth_temp.append(selected_square)  # Add to the temporary labyrinth list
                                    self.correct_squares.append(selected_square)  # Add to the list of correct squares
                                ...
                                if selected_square not in walls:
                                    walls.append(selected_square)  # Add to the list of walls
