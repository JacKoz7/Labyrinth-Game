...
from src.Stage1_Stage2 import First_Stage
from src.Stage3_Stage4 import Second_Stage, Game_status
...
class Labirynt:
    ...
    def game_process(self):
        ...
        Player1_ending = Second_Stage(
            Player1_beginning.treasure,
            Player1_beginning.labyrinth,
            Player1_beginning.cross,
        )

        Player2_beginning.play(self.screen, self.txt2, "Continue ")
        Player2_ending = Second_Stage(
            Player2_beginning.treasure,
        ...
        while Player1_status.winner is False and Player2_status.winner is False:
            if Player1_status.winner is False:
                Player2_ending.endgame(
                    self.screen,
                    self.txt,
                    self.txt2,
                    Player1_status.walls,
                    Player1_status.found_labyrinth,
                    Player2_status.winner,
                )

            if Player2_status.winner is False:
                Player1_ending.endgame(
                    self.screen,
                    self.txt2,
                    self.txt,
                    Player2_status.walls,
                    Player2_status.found_labyrinth,
