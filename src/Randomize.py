...
class RandomizeButton:
    def __init__(self, image, pos):
        ...
        )  # Rect obrazka (dla detekcji kolizji)
        ...
        if position[0] in range(self.rect.left, self.rect.right) and position[
            ...
            for x in range(alpha_mask.get_width()):
                for y in range(alpha_mask.get_height()):
                    ...
                    if (
                        pixel_color[3] > 0
                    ):  # Sprawdź wartość kanału alfa (większa niż 0 oznacza nieprzezroczystość)
                        alpha_mask.set_at(
                            (x, y), (255, 255, 255, pixel_color[3])
                        )  # Ustaw piksel na biały (z zachowaniem wartości alfa)
...
def generate_maze(treasure, cross):
    ...
    while x1 != x2 or y1 != y2:
        ...
        if (
            new_point != path[-1]
        ):  # Sprawdź, czy nowy punkt jest różny od ostatniego punktu na ścieżce
