...
class Button:
    def __init__(self, image, pos, text_input, font, base_color, new_color):
        ...
        self.new_color = new_color  # The altered hue of the text (e.g., upon mouse hover)
        self.text_input = text_input  # The text to be displayed upon the button
        self.text = self.font.render(self.text_input, True, self.base_color)  # Rendering the text with its colour

        # If no image is provided, utilize the rendered text as the button's image
        if self.image is None:
            self.image = self.text

        self.rect = self.image.get_rect(center=(self.x_pos, self.y_pos))  # The rectangle of the image (for collision detection)
        self.text_rect = self.text.get_rect(center=(self.x_pos, self.y_pos))  # The rectangle of the text
            ...
            for x in range(new_image.get_width()):
                for y in range(new_image.get_height()):
                    ...
                    if pixel_color.a > 0:  # Ensuring the pixel's opacity
                        ...
                        new_color.a = pixel_color.a  # Retaining the original opacity value
                        new_image.set_at((x, y), new_color)  # Assigning the new hue to the pixel
