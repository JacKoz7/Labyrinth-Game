...
from src.Randomize import place_treasure, place_cross, generate_maze, place_random_point
...
class First_Stage:
    ...
    def selected_square(self, location, mouse_x, mouse_y):
        if location is not None and (
            100 <= mouse_x <= 100 + 10 * 60 and 50 <= mouse_y <= 50 + 10 * 60
        ):
            row = location[0]
            col = location[1]
            return row, col
    ...
    def play(self, screen, txt, button_text):
        ...
        show_next_move = (
            False  # Whether to show the next move (when the labyrinth is already drawn)
        )
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if (
                    event.type == pygame.MOUSEBUTTONDOWN
                    and treasure_drawn
                    and len(self.labyrinth) < 37
                    and event.button == 1
                ):  # If the left mouse button is depressed.:
                    mouse_x, mouse_y = pygame.mouse.get_pos()

                    selected_labyrinth = self.selected_square(
                    ...
                    if selected_labyrinth is not None:
                        ...
                        for last_square in self.labyrinth:
                            if (
                                selected_labyrinth
                                == (last_square[0] - 1, last_square[1])
                                or selected_labyrinth
                                == (last_square[0] + 1, last_square[1])
                                or selected_labyrinth
                                == (last_square[0], last_square[1] - 1)
                                or selected_labyrinth
                                == (last_square[0], last_square[1] + 1)
                            ):
                                # To avoid adding the selected square and treasure to the labyrinth again
                                if (
                                    selected_labyrinth not in self.labyrinth
                                    and selected_labyrinth != self.labyrinth[0]
                                ):
                ...
                if (
                    ...
                    if (
                        ...
                        mouse_x, mouse_y = pygame.mouse.get_pos()
                        selected_cross = self.selected_square(
                            location, mouse_x, mouse_y
                        )
                ...
                if (
                    event.type == pygame.MOUSEBUTTONDOWN
                    and menu_button.CheckForInput(player_mouse_pos)
                    and event.button == 1
                ):  # If the left mouse button is depressed.:
                ...
                if (
                    event.type == pygame.MOUSEBUTTONDOWN
                    and undo_button.CheckForInput(player_mouse_pos)
                    and return_button.CheckForInput(player_mouse_pos)
                ...
                if show_next_move:
                    if (
                        event.type == pygame.MOUSEBUTTONDOWN
                        and button_player2.CheckForInput(player_mouse_pos)
                        and random_mode
                        and event.button == 1
                    ):  # If the left mouse button is depressed.:
