...
class GameState:
    def __init__(self):
        ...
        self.tick_image = pygame.image.load('Images/green_tick.png')  # The image of a green tick
        self.x_image = pygame.image.load("Images/Red_X.png")  # The image of a red cross
    ...
    def draw_board(self, screen):
        ...
        if 100 <= mouse_x <= 99 + 10 * self.square_size and 50 <= mouse_y <= 49 + 10 * self.square_size:
            selected_col = (mouse_x - 100) // self.square_size  # Calculate the selected column
            selected_row = (mouse_y - 50) // self.square_size  # Calculate the selected row
        ...
        for x in range(5):  # Loop through 5 squares
            ...
            if x < counter:  # If there are fewer marked squares than the counter
                screen.blit(self.tick_image,
                            (900 + x * self.square_size, 330 + self.square_size))  # Draw the green tick
        if wall:
            screen.blit(self.x_image,
                        (900 + counter * self.square_size, 330 + self.square_size))  # Draw the red cross
